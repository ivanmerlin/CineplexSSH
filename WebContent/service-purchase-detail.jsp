...
<body>
	...
	<h4 align="center" style="margin-top:150px;">请选择座位</h4>
...
<div class="center-block btngroup" > 
<button    onclick="cardBuy()">有卡购买</button>
<button    onclick="post_seat()">现金购买</button>
</div>
...
$(function(){
	$.post("get_unavailable_seat2.action",{}, seatcallback, "json");
});
...
function post_seat(){
	...
	if($aa.length>0)
	{
		$.post("select_seat2.action",{'row':$aa.eq(0).attr('data-row'),'col':$aa.eq(0).attr("data-column"),'index':0}, selectcallback, "json");
	}
...
function selectcallback(data){
	...
	{
		$.post("select_seat2.action",{'row':$aa.eq(data.index+1).attr('data-row'),'col':$aa.eq(data.index+1).attr("data-column"),'index':data.index+1}, selectcallback, "json");
	}else{
		location.replace("sellTicketCash.action?num="+$aa.length);
		alert("出票成功");
	}
}
function cardBuy(){
	...
	if($aa.length>0)
	{
		$.post("select_seat2.action",{'row':$aa.eq(0).attr('data-row'),'col':$aa.eq(0).attr("data-column"),'index':0}, selectcallback2, "json");
	}
	
}
function selectcallback2(data){
	var $aa=$("a[seatchecked='checked']");
	if($aa.length>data.index+1)
	{
		$.post("select_seat2.action",{'row':$aa.eq(data.index+1).attr('data-row'),'col':$aa.eq(data.index+1).attr("data-column"),'index':data.index+1}, selectcallback2, "json");
	}else{
		location.replace("sellTicketCard.action?num="+$aa.length);
